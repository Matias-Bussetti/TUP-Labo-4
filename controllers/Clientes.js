  ...
  axios.get(URL)
    .then((apiResponse) => {
      ...
      const filteredData = data.filter(cliente => 
        cliente.Genero.toLowerCase() === genero.toLowerCase());
...
// Función para obtener clientes por nombre (coincidencias parciales)
const getClientesByName = (req = request, res = response) => {
  const { nombre } = req.query;

  if (!nombre) {
    return res.status(400).json(CustomStatusMessage.from(null, 400, 'El parámetro nombre es obligatorio'));
  }
  ...
  axios.get(URL)
    .then((apiResponse) => {
      ...
      // Filtrar por coincidencia parcial (ignorando mayúsculas/minúsculas)
      const filteredData = data.filter(cliente => 
        cliente.nombre.toLowerCase().includes(nombre.toLowerCase())
      );

      if (filteredData.length === 0) {
        return res.status(404).json(CustomStatusMessage.from(null, 404, 'No se encontraron clientes con el nombre especificado'));
      }
